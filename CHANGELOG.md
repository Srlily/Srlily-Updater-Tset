...
- 检查更新结果对话框（退出码与说明）
- 发现新版本时可一键打开更新器界面
- 启动日志 `.srlily-updater/host-launch.log`

### 修复

- 检查更新无任何反应
- 原因：成功时只改状态栏；更新器无输出被隐藏
- 现状：始终弹窗展示结果
- 更新界面连接 GitHub 后自动退出
- 原因：调用了更新器不存在的 `--ui`，参数不符合 CLI 约定
- 现状：界面模式只传 `--root`（与 `--config`）；检查模式 `--check --root`
- 退出码 10（发现新版本）被当作失败
- 现状：0 / 10 / 20 均视为检查流程正常结束

### 优化

- 同步传入 `--config` 指向安装目录 `updater.config.json`
- 捕获更新器 stdout/stderr 写入对话框与日志
...
- 应用内接入 Srlily-Updater：主界面「检查更新」「更新界面」按钮
- CLI：`--check-update`、`--apply-update`、`--update-ui`、`--updater-path`
- 可选参数 `--updater <path>` 指定 Updater.exe
- 环境变量 `SRLILY_UPDATER` 或安装目录 `updater\Updater.exe` 自动解析更新器
- 打包输出包含 `updater.config.json` 宿主配置

### 修复

- 应用进程内此前不会调用更新器，仅支持外部脚本联调

### 优化

- 更新器探测顺序：SRLILY_UPDATER → 安装目录 updater\ → 上级 Srlily-Updater\dist → 本机默认路径
- 缺少 Updater.exe 或宿主配置时给出明确提示
...
- 有实质内容的小节保留标题；无内容的小节整节省略（不写「（无）」）
- Release 说明结构与历史版本保持一致
